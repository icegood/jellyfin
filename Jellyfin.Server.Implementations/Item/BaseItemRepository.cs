    ...
    : IItemRepository
{
    ...
    public ItemCounts GetItemCounts(InternalItemsQuery filter)
    {
        ...
        {
            if (count.Key == (int)BaseItemKind.MusicAlbum)
            {
                result.AlbumCount = count.Count;
            }
            else if (count.Key == (int)BaseItemKind.MusicArtist)
            {
                result.ArtistCount = count.Count;
            }
            else if (count.Key == (int)BaseItemKind.Episode)
            {
                result.EpisodeCount = count.Count;
            }
            else if (count.Key == (int)BaseItemKind.Movie)
            {
                result.MovieCount = count.Count;
            }
            else if (count.Key == (int)BaseItemKind.MusicVideo)
            {
                result.MusicVideoCount = count.Count;
            }
            else if (count.Key == (int)BaseItemKind.LiveTvProgram)
            {
                result.ProgramCount = count.Count;
            }
            else if (count.Key == (int)BaseItemKind.Series)
            {
                result.SeriesCount = count.Count;
            }
            else if (count.Key == (int)BaseItemKind.Audio)
            {
                result.SongCount = count.Count;
            }
            else if (count.Key == (int)BaseItemKind.Trailer)
            {
